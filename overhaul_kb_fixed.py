...
if start_pos != -1 and end_pos != -1:
    new_func = r"""        function formatKbAnswer(text) {
            ...
            cleaned = cleaned.replace(/([^\\n])\s*(\*\*?(?:Prerequisites|Instructions for|Steps to|Instructions|Note|What is|Conclusion|Summary|Post-Closing|Troubleshooting):?\*\*?)/gi, '$1\n\n$2');
            cleaned = cleaned.replace(/([^\\n])\s+(\d+\.\s*)/g, '$1\n\n$2');
            cleaned = cleaned.replace(/([^\\n])\s+([-•*])\s+(?!\*)/g, '$1\n\n$2');
            ...
            blocks.forEach(block => {
                ...
                // Highlight clue terms in red color (text within quotes or specific button patterns)
