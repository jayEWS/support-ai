...
# 1. New JS Function
new_js_func = """        function formatKbAnswer(text) {
            if (!text) return '';
            ...
            for (let i = 0; i < rawLines.length; i++) {
                ...
                if (match = line.match(/^(\\d+)\\.\\s*(.*)/)) {
                    ...
                    type = 'bullet';
                } else if (line.match(/^###\\s+/) || line.match(/^\\*\\*.*?\\*\\*:?$/) || (line.startsWith('**') && line.length < 60 && (line.endsWith('**') || line.endsWith('**:')))) {
                    type = 'header';
                }
...
if start_pos != -1 and end_pos != -1:
    content = content[:start_pos] + new_js_func + "\n\n        " + content[end_pos:]
    print("JS Function updated.")
else:
...
css_start_pat = ".kb-answer .kb-numbered {"
css_end_pat = ".kb-answer .kb-para {" # Use next class as end marker

css_start_pos = content.find(css_start_pat)
css_end_pos = content.find(css_end_pat)

if css_start_pos != -1 and css_end_pos != -1:
    content = content[:css_start_pos] + css_updates + "\\n\\n        " + content[css_end_pos:]
    # Fix the double backslash from my string literal
...
with open(filepath, 'w', encoding='utf-8') as f:
    f.write(content)
print("KB overhaul 2.0 complete.")
