...
# ============ Freshdesk Historical Data API ============

@app.get("/api/freshdesk/stats")
async def freshdesk_stats():
    """Get aggregated stats from Freshdesk historical data."""
    db = _require_db()
    return db.get_freshdesk_stats()

@app.get("/api/freshdesk/contacts")
async def freshdesk_contacts(search: str = None, page: int = 1, per_page: int = 50):
    """Get Freshdesk contacts with optional search and pagination."""
    db = _require_db()
    return db.get_freshdesk_contacts(search=search, page=page, per_page=per_page)

@app.get("/api/freshdesk/contacts/{freshdesk_id}")
async def freshdesk_contact_detail(freshdesk_id: str):
    """Get a single Freshdesk contact."""
    db = _require_db()
    contact = db.get_freshdesk_contact(freshdesk_id)
    if not contact:
        raise HTTPException(status_code=404, detail="Contact not found")
    return contact

@app.get("/api/freshdesk/contacts/{freshdesk_id}/tickets")
async def freshdesk_contact_tickets(freshdesk_id: str, page: int = 1, per_page: int = 50):
    """Get tickets for a specific Freshdesk contact."""
    db = _require_db()
    return db.get_freshdesk_tickets(contact_id=freshdesk_id, page=page, per_page=per_page)

@app.get("/api/freshdesk/tickets")
async def freshdesk_tickets(search: str = None, ticket_type: str = None,
                             status: str = None, contact_id: str = None,
                             page: int = 1, per_page: int = 50):
    """Get Freshdesk tickets with filters and pagination."""
    db = _require_db()
    return db.get_freshdesk_tickets(contact_id=contact_id, search=search,
                                    ticket_type=ticket_type, status=status,
                                    page=page, per_page=per_page)

@app.post("/api/freshdesk/link-contact")
async def link_freshdesk_contact(request: Request):
    """Link a Freshdesk contact to an internal portal user."""
    db = _require_db()
    data = await request.json()
    freshdesk_id = data.get("freshdesk_id")
    user_id = data.get("user_id")
    if not freshdesk_id or not user_id:
        raise HTTPException(status_code=400, detail="freshdesk_id and user_id required")
    ok = db.link_freshdesk_contact_to_user(freshdesk_id, user_id)
    if not ok:
        raise HTTPException(status_code=404, detail="Contact not found")
    return {"status": "linked", "freshdesk_id": freshdesk_id, "user_id": user_id}

@app.get("/api/freshdesk/user-history/{user_id}")
async def freshdesk_user_history(user_id: str):
    """Get Freshdesk history for a linked internal user."""
    db = _require_db()
    history = db.get_freshdesk_history_for_user(user_id)
    if not history:
        return {"contact": None, "tickets": []}
    return history

@app.post("/api/freshdesk/import")
async def freshdesk_import_endpoint(request: Request, background_tasks: BackgroundTasks):
    """Trigger Freshdesk import from uploaded Excel files or server folder."""
    db = _require_db()
    data = await request.json()
    folder = data.get("folder", "Customer")
