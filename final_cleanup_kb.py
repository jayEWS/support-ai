...
if start_pos != -1 and end_pos != -1:
    new_func = r"""        function formatKbAnswer(text) {
            ...
            // 1. CLEANUP (Initial sanitization)
            let cleaned = text
                ...
                .replace(/\[\s*[,\s]*\s*\]/g, '')
                .replace(/,\s*,/g, ',')
                .replace(/\* \* \* \*/g, '') // Strip trailing AI artifacts
                .replace(/,\s*$/gm, '');

            // 2. STRUCTURAL ISOLATION
            // Ensure bullets have a space
            ...
            for (let i = 0; i < rawLines.length; i++) {
                ...
                // Strip random trailing AI stars from lines
            ...
            blocks.forEach(block => {
                ...
                if (block.type === 'header') {
                    ...
                    if (inList) { html += '</ul>'; inList = false; }
                    // Strip leftover raw bullet symbols if they somehow got into a para block
                    let finalPara = text.replace(/^([-•*])\s+/, '');
                    html += `<p class="kb-para">${finalPara}</p>`;
                }
            });
    ...
    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(new_content)
    print("KB Final Artifact Cleanup applied.")
else:
